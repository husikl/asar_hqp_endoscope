...
class interaction_control:
    ...
    def on_right_click(self, event=None):
        ...
        self.anchors.append(np.array([[self.right_click_x, self.right_click_y]]))
        print("right click:", self.right_click_x, self.right_click_y)
        ...
        if len(self.boxes) == 1:
            ...
            if len(self.anchors) == len(self.boxes):
                for i, i_box in enumerate(self.boxes):
                    ...
                    trans_boxes = np.array(i_box)
                    masks, _, _ = self.sam.predict(
                        point_coords=self.anchors[
                            i
                        ],  # np.array([[int(trans_boxes[2]-trans_boxes[0]), int(trans_boxes[3]-trans_boxes[1])]]),
                    ...
                    cv2.imwrite(f"mask_to_track{i}.png", masks[0] * 255)
    ...
    def do_interact(self, image=None, tk_root=None):
        ...
        if self.image is not None:
            ...
            self.l.bind("<Button 3>", self.on_right_click)
            self.l.pack()
            self.l.configure(image=self.photo)
