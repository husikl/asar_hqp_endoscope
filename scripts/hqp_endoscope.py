...
def CheckPoseInSafetyRegion(se3pose):
    # Given safety cube dimensions (replace these with actual values)
    x_min, x_max = 0, 10
    y_min, y_max = 0, 10
    z_min, z_max = 0, 10
    
    # Extract translation vector from SE3 pose
    translation_vector = se3pose.translation
    
    x, y, z = translation_vector[0], translation_vector[1], translation_vector[2]
    
    # Check if the point is within the safety cube
...
class HqpEndoscopeController:
    def __init__(self, urdf_path, params):
        ...
        self.target_depth = 0.09
        self.init_camera_z = 0.07
        # Create ik tracking control service:
        self.track_service = rospy.Service("/fov_control", Empty, self.trackControlCb)
    ...
    def compute_residual_depth(self):
        d_des = self.target_depth
    ...
    def compute_residual_VT(self):
        ...
        target = np.array([self.roi_xy[1], -self.roi_xy[0]])
        # res_vt = -self.roi_xy
        res_vt = -target
    ...
    def sendOperSpaceTrajGoalToActionServer(self, q_sol):
        ...
        client.send_goal(goal)
        client.wait_for_result(0.001)
        # result = client.get_result()
        rospy.loginfo("goal sent to action server")

    def getGreenObjCb(self, msg):
        self.roi_xy = np.array([msg.x, msg.y])
        self.roi_radius = msg.z
        self.object_detected = True
        # print("has green obj :", self.roi_xy)
    ...
    def updateDepth(self, roi_radius):
        # Define min and max radius
        min_radius = 400
        max_radius = 500

        # Define min and max depth in meters
        min_depth = 0.06  # 5cm
        max_depth = 0.12  # 11cm
        
        # Adjust target depth based on the roi_radius
        if roi_radius < min_radius and self.target_depth > min_depth:
            self.target_depth += 0.001
            # self.init_camera_z += 0.001
        elif roi_radius > max_radius and self.target_depth < max_depth:
            self.target_depth -= 0.001
            # self.init_camera_z -= 0.001
        

        # Make sure target_depth stays within min and max depth limits
        self.target_depth = min(max(self.target_depth, min_depth), max_depth)

        return self.target_depth

    def trackControlCb(self, req):
        if self.start_tracking == False:
    ...
    def solveIK_VT(self, q_init):
        ...
        self.updateDepth(self.roi_radius)
        res_depth, err_depth, lhat = self.compute_residual_depth()
        ...
        for it in range(1):
            ...
            x_max = np.array([0.1, 0.1, 0.1, 0.1])
            x_min = np.array([-0.1, -0.1, -0.1, -0.1])
            # print("x_max: ", x_max)
            # print("x_min: ", x_min)
...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        ...
        if controller_ready:
            ...
            if solver.start_tracking and solver.object_detected:
...
# Endoscope init joint values deg : 126.54, 314.57, 269.87, 65.30, 295.53, 112.18, 14.29
# %%
